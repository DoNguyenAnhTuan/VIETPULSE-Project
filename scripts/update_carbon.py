...
OUTPUT_CSV = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'client', 'src', 'data', 'electric_async1.csv')
...
def append_to_csv(row_dict):
    full_row = {col: row_dict.get(col, 0) for col in CSV_COLUMNS[:-2]}
    full_row["Carboon (tấn CO₂)"] = compute_carboon(full_row)
    full_row["Total MWh"] = compute_total_mwh(full_row)
    ...
    # ❗❗ NEW: Check if date already exists
    if os.path.exists(OUTPUT_CSV):
        existing = pd.read_csv(OUTPUT_CSV, on_bad_lines='skip')
        if "Day" in existing.columns:
            if day_val in existing["Day"].astype(str).values:
                print(f"[SKIP] Already exists: {day_val}")
                return
...
if __name__ == "__main__":
    ...
    #     writer = csv.DictWriter(f, fieldnames=CSV_COLUMNS)
    #     writer.writeheader()
    # print(f"[RESET] Đã tạo lại file {OUTPUT_CSV} với header mới.")
